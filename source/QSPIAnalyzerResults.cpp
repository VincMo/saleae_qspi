...
QSPIAnalyzerResults::QSPIAnalyzerResults(QSPIAnalyzer *analyzer,
                                         QSPIAnalyzerSettings *settings)
    : AnalyzerResults(), mSettings(settings), mAnalyzer(analyzer) {}

QSPIAnalyzerResults::~QSPIAnalyzerResults() {}

void QSPIAnalyzerResults::GenerateBubbleText(U64 frame_index, Channel &channel,
  ...
  switch (frame.mType) {
    case FrameTypeCommand: {
      ...
      ss << "Command: " << number_str << " "
         << GetQSPICommandAttr(frame.mData1).CommandName;
      AddResultString(ss.str().c_str());
    } break;
...
void QSPIAnalyzerResults::GenerateExportFile(const char *file,
                                             DisplayBase display_base,
  ...
  ss << "time,value,type,command" << std::endl;
...
void QSPIAnalyzerResults::GenerateFrameTabularText(U64 frame_index,
                                                   DisplayBase display_base) {
  ClearTabularText();
  Frame frame = GetFrame(frame_index);
...
void QSPIAnalyzerResults::GeneratePacketTabularText(U64 packet_id,
                                                    DisplayBase display_base) {
  ClearResultStrings();
  AddResultString("not supported");
}

void QSPIAnalyzerResults::GenerateTransactionTabularText(
