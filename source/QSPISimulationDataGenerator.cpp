...
QSPISimulationDataGenerator::QSPISimulationDataGenerator()
    : mSerialText("garbage!"), mStringIndex(0) {}

QSPISimulationDataGenerator::~QSPISimulationDataGenerator() {}

void QSPISimulationDataGenerator::Initialize(U32 simulation_sample_rate,
                                             QSPIAnalyzerSettings* settings) {
  ...
  mClock = mQSPISimulationChannels.Add(settings->mClockChannel, mSimulationSampleRateHz,
                                       mSettings->mClockInactiveState);
  mEnable = mQSPISimulationChannels.Add(settings->mEnableChannel, mSimulationSampleRateHz, BIT_LOW);

  mQSPISimulationChannels.AdvanceAll(
      mClockGenerator.AdvanceByHalfPeriod(10.0));  // insert 10 bit-periods of idle

  mValue = 0;
}

U32 QSPISimulationDataGenerator::GenerateSimulationData(
  ...
  while (mClock->GetCurrentSampleNumber() < adjusted_largest_sample_requested) {
    for (U64 i = 0; i < 40; i++) {
      ...
      CreateQSPITransaction(GetQSPICommand(i), 0xBEADED, dataarray, sizeof(dataarray) / sizeof(U64),
                            mSettings->mModeState);
...
void QSPISimulationDataGenerator::CreateQSPITransaction(U64 command, U64 address, U64 data[],
                                                        int datasize, int modestate) {
  // Begin transaction
  if (mEnable != NULL) mEnable->TransitionIfNeeded(BIT_LOW);
  ...
  switch (modestate) {
    case 1:
      OutputWord(command, 0x01);  // Extended mode
      break;
    case 2:
      OutputWord(command, 0x03);  // Dual mode
      break;
    case 3:
      OutputWord(command, 0x0F);  // Quad mode
      break;
  }
  ...
  if (GetQSPICommandAttr(command).UsesDummyCycles) {
    // OutputWord(0x00, 0x01);  // potentially a problem.  this needs to be 8
    // clock cycles not 8 bits.  some commands require 10 clock cycles.
...
void QSPISimulationDataGenerator::OutputWord(U64 data, int pinmask) {
  ...
  switch (pinmask) {  // pin mask defines which pins are used for the operation
    ...
    case 0x02:  // using DQ1
      for (U32 i = 0; i < 8; i++) {
        if (mDQ1 != NULL) mDQ1->TransitionIfNeeded(data_bits.GetNextBit());
    ...
    case 0x0F:  // using DQ0 through DQ3
      for (U32 i = 0; i < 2; i++) {
        if (mDQ3 != NULL) mDQ3->TransitionIfNeeded(data_bits.GetNextBit());
        if (mDQ2 != NULL) mDQ2->TransitionIfNeeded(data_bits.GetNextBit());
        if (mDQ1 != NULL) mDQ1->TransitionIfNeeded(data_bits.GetNextBit());
        if (mDQ0 != NULL) mDQ0->TransitionIfNeeded(data_bits.GetNextBit());
  ...
  // Set all lines low
  if (mDQ0 != NULL) mDQ0->TransitionIfNeeded(BIT_LOW);

  if (mDQ1 != NULL) mDQ1->TransitionIfNeeded(BIT_LOW);

  if (mDQ2 != NULL) mDQ2->TransitionIfNeeded(BIT_LOW);

  if (mDQ3 != NULL) mDQ3->TransitionIfNeeded(BIT_LOW);
