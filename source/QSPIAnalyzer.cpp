...
QSPIAnalyzer::~QSPIAnalyzer() { KillThread(); }
...
void QSPIAnalyzer::AdvanceToActiveEnableEdgeWithCorrectClockPolarity() {
  ...
  for (;;) {
    if (IsInitialClockPolarityCorrect() ==
        true)  // if false, this function moves to the next active enable edge.
      break;
  }
}
...
bool QSPIAnalyzer::IsInitialClockPolarityCorrect() {
  if (mClock->GetBitState() == mSettings->mClockInactiveState) return true;
  ...
  if (mEnable != NULL) {
    ...
    mClock->AdvanceToNextEdge();  // at least start with the clock in the idle
                                  // state.
    mCurrentSample = mClock->GetSampleNumber();
    return true;
  }
...
void QSPIAnalyzer::GetFrame() {
  ...
  switch (mSettings->mModeState) {
    case 1:
      currentCommand = GetCommand(0x01);  // Extended mode
      break;
    case 2:
      currentCommand = GetCommand(0x03);  // Dual mode
      break;
    case 3:
      currentCommand = GetCommand(0x0F);  // Quad mode
      break;
  }
  ...
  if (IsParseResultError(currentCommand)) {
    ...
    if (IsCommandValid(currentCommand.data) == false) {  // if command byte is not valid, skip
                                                         // forward to end of active edge
      AdvanceToActiveEnableEdgeWithCorrectClockPolarity();
      return;
    }
  }
  ...
  if (currentCommandAttr.AcceptsAddr) {
    ...
    switch (mSettings->mModeState) {
      case 1:
        currentAddress = GetAddress(currentCommandAttr.AddressLineMask);  // Extended mode
        break;
      case 2:
        currentAddress = GetAddress(0x03);  // Dual mode
        break;
      case 3:
  ...
  if (currentCommandAttr.HasData) {
    for (;;) {
      ...
      switch (mSettings->mModeState) {
        case 1:
          currentData = GetData(currentCommandAttr.DataLineMask);  // Extended mode
          break;
        case 2:
          currentData = GetData(0x03);  // Dual mode
          break;
        case 3:
...
QSPIAnalyzer::ParseResult QSPIAnalyzer::GetDummy() {
  ...
  for (U32 i = 0; i < mSettings->mDummyCycles; i++) {
    ...
    // note that we can't just advance the enable line to the next edge, becuase
    // there may not be another edge

    if (WouldAdvancingTheClockToggleEnable() == true) {
      AdvanceToActiveEnableEdgeWithCorrectClockPolarity();  // ok, we pretty
                                                            // much need to
                                                            // reset everything
                                                            // and return.
    ...
    if (i == 0) first_sample = mClock->GetSampleNumber();
...
QSPIAnalyzer::ParseResult QSPIAnalyzer::GetData(U64 DataLineMask) {
  ...
  data_result.Reset(&data_word, AnalyzerEnums::MsbFirst,
                    8);  // todo: change 8 to bitspertransfer?
...
QSPIAnalyzer::ParseResult QSPIAnalyzer::GetAddress(U64 AddressLineMask) {
  ...
  address_result.Reset(&address_word, AnalyzerEnums::MsbFirst,
                       (mSettings->mAddressSize) * 8);  // todo: change 8 to bitspertransfer?
...
QSPIAnalyzer::ParseResult QSPIAnalyzer::GetCommand(U64 CommandLineMask) {
  ...
  command_result.Reset(&command_word, AnalyzerEnums::MsbFirst,
                       8);  // todo: change 8 to bitspertransfer?
...
void QSPIAnalyzer::SaveResults(QSPIAnalyzer::ParseResult return_value, QSPIFrameType frame_type) {
  ...
  //	mResults->AddMarker(mArrowLocations[i], mArrowMarker,
...
bool QSPIAnalyzer::NeedsRerun() { return false; }

U32 QSPIAnalyzer::GenerateSimulationData(U64 minimum_sample_index, U32 device_sample_rate,
...
U32 QSPIAnalyzer::GetMinimumSampleRateHz() {
  return 10000;  // we don't have any idea, depends on the SPI rate, etc.;
                 // return the lowest rate.
}

const char* QSPIAnalyzer::GetAnalyzerName() const { return "QSPI"; }

const char* GetAnalyzerName() { return "QSPI"; }

Analyzer* CreateAnalyzer() { return new QSPIAnalyzer(); }
