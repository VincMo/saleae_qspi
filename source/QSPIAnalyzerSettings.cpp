...

{
  ...
  mClockInactiveStateInterface->AddNumber(BIT_LOW,
                                          "Clock is Low when inactive (CPOL = 0, CPHA = 0)",
                                          "CPOL = 0 (Clock Polarity), CPHA = 0 (Clock Phase)");
  mClockInactiveStateInterface->AddNumber(BIT_HIGH,
                                          "Clock is High when inactive (CPOL = 1, CPHA = 1)",
                                          "CPOL = 1 (Clock Polarity), CPHA = 1 (Clock Phase)");
  ...
  mModeStateInterface->AddNumber(1, "Extended",
                                 "Extended mode (uses DQ0 for command and "
                                 "DQ[3:0] to data depending on command)");
...
bool QSPIAnalyzerSettings::SetSettingsFromInterfaces() {
  ...
  if ((dq0 == UNDEFINED_CHANNEL) && (dq1 == UNDEFINED_CHANNEL) && (dq2 == UNDEFINED_CHANNEL) &&
      (dq3 == UNDEFINED_CHANNEL) && (clock == UNDEFINED_CHANNEL) && (enable == UNDEFINED_CHANNEL)) {
